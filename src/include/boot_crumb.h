 ...
 * (misc/memory_map_rp2350.ld) has a `.scratch_x_noinit (NOLOAD)` section
 ...
 * `picotool save -r <addr> <addr+64> file.bin`. picotool's PICOBOOT read can
 ...
 *   [5] tracer (main.c) — 0xAAAAAAAA before check, 0xBBBBBBBB after
 *   [6] tracer (boot_crumb.c) — 0xC0DE0001 reached check, 0xC0DE0002 BOOTSEL
...
#define BOOT_CRUMB_SLOT_CORE0_TASK 4
#define BOOT_CRUMB_SLOT_CORE1_TASK 7
...
/* Backing store lives in the .scratch_x_noinit NOLOAD section so the C
...
enum boot_phase {
    ...
    PHASE_BEFORE_BT_HID_INIT      = 0x13,
    PHASE_BT_HID_L2CAP_DONE       = 0x14,
    PHASE_BT_HID_HOST_INIT_DONE   = 0x15,
    PHASE_BT_HID_FW_PRELOAD_DONE  = 0x16,
    PHASE_AFTER_BT_HID_INIT       = 0x17,
    PHASE_BEFORE_WATCHDOG_ENABLE  = 0x18,
    PHASE_AFTER_WATCHDOG_ENABLE   = 0x19,
    PHASE_BEFORE_SET_ACTIVE       = 0x1A,
    PHASE_AFTER_SET_ACTIVE        = 0x1B,
    PHASE_MAIN_LOOP_FIRST_ITER    = 0x1C,
    PHASE_MAIN_LOOP_RUNNING       = 0x1D,
};
